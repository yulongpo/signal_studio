...
std::string serialize_manifest(const std::vector<SampleRecord>& records) {
  ...
  for (std::size_t i = 0; i < records.size(); ++i) {
    ...
    out << "{\"sampleId\":\"" << json_escape(r.sample_id) << "\",\"dataPath\":\""
        ...
        << "\",\"sourceFormat\":\"" << json_escape(r.source_format) << "\",\"sampleCount\":" << r.sample_count
        << ",\"sampleRateHz\":" << r.sample_rate_hz << ",\"sha256\":\"" << json_escape(r.sha256_digest) << "\"}";
  }
  out << "]}";
  return out.str();
}
...
std::vector<SampleRecord> JsonFileDataset::query(const SampleQuery& q) const {
  ...
  for (const auto& r : records_) {
    if (q.label && r.label != *q.label)
      continue;
    if (q.source_format && r.source_format != *q.source_format)
      continue;
    out.push_back(r);
