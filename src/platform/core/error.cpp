...
constexpr ErrorCategory category_for(ErrorReason reason) noexcept {
  switch (reason) {
  case ErrorReason::invalid_argument:
  ...
  case ErrorReason::internal_failure:
...
StatusInvariant validate_failure_invariants(const ErrorCode& code, const ErrorDetails& details) noexcept {
  if (code.is_success())
    return StatusInvariant::success_code;
  if (!valid_domain(code.domain))
    return StatusInvariant::invalid_domain;
  if (!valid_reason(code.reason))
    return StatusInvariant::invalid_reason;
  if (details.category != category_for(code.reason))
    return StatusInvariant::category_mismatch;
  if (!valid_severity(details.severity))
    return StatusInvariant::invalid_severity;
  if (details.user_message.empty())
    return StatusInvariant::missing_user_message;
  if (details.recovery_actions.empty())
    return StatusInvariant::missing_recovery_action;
  for (const auto& action : details.recovery_actions) {
    if (action.empty())
      return StatusInvariant::empty_recovery_action;
  }
  if (details.retryable && code.reason != ErrorReason::unavailable && code.reason != ErrorReason::internal_failure) {
    return StatusInvariant::invalid_retryability;
  }
  if (details.cause_chain.size() > max_error_cause_depth)
    return StatusInvariant::cause_depth_exceeded;
  for (const auto& cause : details.cause_chain) {
    ...
    if (cause.category != category_for(cause.code.reason))
      return StatusInvariant::cause_category_mismatch;
    if (cause.technical_details.empty())
      return StatusInvariant::missing_cause_details;
  }
  for (const auto& reference : details.metrics_refs) {
    if (reference.empty())
      return StatusInvariant::empty_metrics_reference;
  }
...
std::string_view invariant_message(StatusInvariant invariant) noexcept {
  switch (invariant) {
  case StatusInvariant::valid:
    return "valid";
  case StatusInvariant::success_code:
    return "failure status requires a non-success code";
  case StatusInvariant::invalid_domain:
    return "error domain is outside BL1.0";
  case StatusInvariant::invalid_reason:
    return "error reason is outside BL1.0";
  case StatusInvariant::category_mismatch:
    return "error category does not match the stable reason";
  case StatusInvariant::invalid_severity:
    return "error severity is outside BL1.0";
  case StatusInvariant::missing_user_message:
    return "user_message is required";
  case StatusInvariant::missing_recovery_action:
    return "recovery_actions is required";
  case StatusInvariant::empty_recovery_action:
    return "recovery_actions cannot contain an empty action";
  case StatusInvariant::invalid_retryability:
    return "only unavailable or internal failures may be retryable";
  case StatusInvariant::cause_depth_exceeded:
    return "cause_chain exceeds the BL1.0 depth limit";
  case StatusInvariant::invalid_cause_code:
    return "cause_chain contains an invalid failure code";
  case StatusInvariant::cause_category_mismatch:
    return "cause_chain category does not match its stable reason";
  case StatusInvariant::missing_cause_details:
    return "cause_chain technical details are required";
  case StatusInvariant::empty_metrics_reference:
    return "metrics_refs cannot contain an empty reference";
  }
...
Status validate_error(const ErrorCode& code, const ErrorDetails& details) {
  ...
  return invariant == StatusInvariant::valid
             ? Status::success()
             : Status::failure({ErrorDomain::core, ErrorReason::invalid_argument}, "Structured error validation failed",
...
Status Status::failure(ErrorCode code, ErrorDetails details) {
  ...
  if (invariant != StatusInvariant::valid)
    throw std::invalid_argument(std::string{invariant_message(invariant)});
...
const ErrorDetails& Status::details() const noexcept {
  return details_;
}
std::string_view Status::message() const noexcept {
  return details_.user_message;
}
std::string_view Status::diagnostic() const noexcept {
  return details_.technical_details;
}
...
Status Status::with_context(std::string_view context) const {
  ...
  propagated.cause_chain.push_back(
      {code_, details_.category,
       details_.technical_details.empty() ? details_.user_message : details_.technical_details});
  propagated.technical_details =
      std::string{context} + (details_.technical_details.empty() ? "" : ": " + details_.technical_details);
...
std::string Status::serialize_json() const {
  ...
  output << "{\"code\":\"" << code_.stable_text() << "\",\"category\":\"" << error_category_name(details_.category)
         ...
         << escape_json(details_.user_message) << "\",\"technicalDetails\":\""
         << escape_json(details_.technical_details) << "\",\"recoveryActions\":";
  append_strings(output, details_.recovery_actions);
  output << ",\"retryable\":" << (details_.retryable ? "true" : "false") << ",\"taskId\":\""
         << escape_json(details_.task_id) << "\",\"objectId\":\"" << escape_json(details_.object_id)
...
std::string_view error_domain_name(ErrorDomain domain) noexcept {
  ...
  case ErrorDomain::data:
    return "data";
  case ErrorDomain::dsp:
    return "dsp";
  case ErrorDomain::compute:
    return "compute";
  case ErrorDomain::task_runtime:
    return "task-runtime";
  case ErrorDomain::visualization:
    return "visualization";
  case ErrorDomain::workbench:
    return "workbench";
  case ErrorDomain::plugin_sdk:
    return "plugin-sdk";
  case ErrorDomain::model_runtime:
    return "model-runtime";
  case ErrorDomain::dataset:
    return "dataset";
  }
  return "unknown";
}
