...
private:
  [[nodiscard]] static std::string serialize(const Emission& emission) {
    ...
    std::ostringstream stream;
    stream << "SSTJ2" << '\t' << status.revision << '\t' << hex_encode(status.task_id.value) << '\t'
...
public:
  ...
  [[nodiscard]] core::Result<TaskHandle> retry(const TaskId& task_id) {
    ...
    TaskHandle handle;
    {
      ...
      if (source->status.attempt >= source->spec.max_attempts) {
        return task_error(core::ErrorReason::unavailable, "任务已达到最大尝试次数");
      }
      if (!source->work) {
        return task_error(core::ErrorReason::unavailable, "恢复的任务没有可执行工作体");
      }
      ...
      const auto existing = idempotency_index_.find(retry_key);
      if (existing != idempotency_index_.end()) {
        return TaskHandle{shared_from_this(), existing->second};
      }
      auto spec = source->spec;
      spec.task_id = TaskId::generate();
      spec.idempotency_key = retry_key;
      auto record = make_record(std::move(spec), source->work, next_attempt, source->base_idempotency_key);
      const auto id = record->status.task_id.value;
      idempotency_index_[record->spec.idempotency_key] = record->status.task_id;
      tasks_.emplace(id, record);
      if (const auto failed_dependency = first_failed_dependency_locked(*record)) {
        transition_locked(*record, TaskState::dependency_failed, "重试依赖任务失败",
                          dependency_failure(*failed_dependency));
      }
      handle = TaskHandle{shared_from_this(), record->status.task_id};
  ...
  [[nodiscard]] ViewRequestId issue_view_request(std::string scope) {
    ...
    ViewRequestId request{std::move(scope), generation};
    {
      ...
      current_views_[request.scope] = request.generation;
    }
  ...
  [[nodiscard]] core::Status commit_artifact(const TaskId& task_id, const std::filesystem::path& temporary,
    ...
    std::error_code error;
    if (temporary.empty() || final_path.empty() || !std::filesystem::is_regular_file(temporary, error) || error ||
        std::filesystem::exists(final_path, error)) {
      return task_error(core::ErrorReason::invalid_argument, "制品临时路径无效或目标已存在");
    }
...
private:
  [[nodiscard]] core::Status validate_definition(const TaskDefinition& definition) const {
    ...
    if (!definition.work || spec.task_type.empty() || spec.idempotency_key.empty() ||
        spec.provenance.project_id.value.empty() || spec.provenance.data_source_version_id.value.empty() ||
        spec.provenance.source_object.type.empty() || spec.provenance.source_object.id.empty() ||
        spec.max_attempts == 0U || spec.timeout < std::chrono::milliseconds::zero()) {
      return task_error(core::ErrorReason::invalid_argument, "任务定义缺少工作体、幂等键或完整来源链");
    }
  ...
  [[nodiscard]] Emission emission_locked(const Record& record) const {
    return {{0U, record.status},
  ...
  void publish(Emission emission) {
    emission.event.sequence = next_event_sequence_.fetch_add(1U);
    static_cast<void>(history_store_.append(emission));
    ...
    for (const auto& observer : observers) {
      observer->on_event(emission.event);
    }
  }
...
SchedulingRequirement evaluate_scheduling(WorkClass work_class, std::chrono::milliseconds expected_duration) noexcept {
  ...
  return {task_class && expected_duration > std::chrono::milliseconds{50}, expected_duration, work_class};
}
