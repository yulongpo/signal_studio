...
core::Result<std::vector<double>> generate_window(const WindowSpec& spec) {
  ...
  for (std::uint64_t i = 0; i < n; ++i) {
    ...
    case WindowType::hamming:
      ...
      break;
    case WindowType::blackman:
      value = 0.42 - 0.5 * std::cos(2.0 * std::numbers::pi * t) + 0.08 * std::cos(4.0 * std::numbers::pi * t);
      break;
    case WindowType::blackman_harris:
      value = 0.35875 - 0.48829 * std::cos(2.0 * std::numbers::pi * t) +
              0.14128 * std::cos(4.0 * std::numbers::pi * t) - 0.01168 * std::cos(6.0 * std::numbers::pi * t);
      break;
    case WindowType::flattop:
      value = 0.21557895 - 0.41663158 * std::cos(2.0 * std::numbers::pi * t) +
              0.277263158 * std::cos(4.0 * std::numbers::pi * t) - 0.083578947 * std::cos(6.0 * std::numbers::pi * t) +
...
double window_coherent_gain(std::string_view window_name) noexcept {
  ...
  if (window_name == "blackman")
    return 0.42;
  if (window_name == "blackman-harris")
    return 0.35875;
