...
bool valid_tile_kind(TileKind kind) noexcept {
  return kind == TileKind::spectrum_summary || kind == TileKind::stft;
}

bool valid_progressive_state(ProgressiveIndexState state) noexcept {
  return state == ProgressiveIndexState::time_frame_ready || state == ProgressiveIndexState::sample_overview ||
         state == ProgressiveIndexState::building || state == ProgressiveIndexState::complete ||
         state == ProgressiveIndexState::degraded;
}

core::Status validate_tile_compatibility(const CacheKey& key, const Tile& tile) {
  const auto key_validation = key.validate();
  if (!key_validation) {
    return key_validation;
  }
  const auto expected_height = key.tile_kind == TileKind::stft ? key.pixel_height : 1U;
  if (!valid_tile_kind(tile.kind) || tile.kind != key.tile_kind || tile.time_range != key.time_viewport ||
      tile.frequency_begin_hz != key.frequency_begin_hz || tile.frequency_end_hz != key.frequency_end_hz ||
      tile.width != key.pixel_width || tile.height != expected_height ||
      tile.width > std::numeric_limits<std::uint64_t>::max() / tile.height ||
      static_cast<std::uint64_t>(tile.width) * tile.height != tile.values.size()) {
    return data_error(core::ErrorReason::invalid_argument, "Tile metadata or shape is incompatible with its cache key");
  }
...
core::Result<std::vector<std::byte>> encode_tile(const CacheKey& key, const Tile& tile) {
  const auto validation = validate_tile_compatibility(key, tile);
  if (!validation)
    return validation;
...
core::Result<Tile> decode_tile(const CacheKey& key, std::span<const std::byte> bytes) {
  ...
  if (const auto validation = validate_tile_compatibility(key, tile); !validation) {
    return validation;
  }
  return tile;
}
...
bool corrupt_error(const core::Status& status) {
  ...
  return message.find("artifact") != std::string::npos || message.find("digest") != std::string::npos ||
         message.find("incompatible") != std::string::npos || message.find("Tile metadata") != std::string::npos;
}
...
core::Status ProgressiveIndexStatus::transition(ProgressiveIndexState next, double coverage, std::string reason) {
  if (!valid_progressive_state(next) || !valid_progressive_state(state_) || !std::isfinite(coverage) ||
      coverage < 0.0 || coverage > 1.0 || (next == ProgressiveIndexState::complete && coverage != 1.0) ||
...
core::Status CacheKey::validate() const {
  if (!valid_tile_kind(tile_kind) || source_fingerprint.empty() || data_source_version_id.empty() ||
      descriptor_digest.empty() || algorithm_version.empty() || dependency_version.empty() ||
      parameter_digest.empty() || quality.empty() || !loaded_range.contains(time_viewport) || time_viewport.empty() ||
      frequency_end_hz <= frequency_begin_hz || pixel_width == 0U || pixel_height == 0U) {
...
core::Result<Tile> DirectDftTileProducer::produce(const TileRequest& request, const CancellationToken& cancellation) {
  ...
  const auto bins = static_cast<std::uint64_t>(request.key.pixel_width);
  if (input_count == 0U || input_count > maximum_input_samples_ || rows == 0U || rows > input_count ||
      bins > maximum_operations_ / input_count ||
      static_cast<std::uint64_t>(rows) > maximum_operations_ / (input_count * bins) ||
      static_cast<std::uint64_t>(rows) >
          static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max()) / request.key.pixel_width) {
...
std::shared_ptr<const Tile> MemoryTileCache::get(const CacheKey& key) {
  ...
  if (!iterator->second.tile || !validate_tile_compatibility(key, *iterator->second.tile).ok()) {
...
core::Result<std::shared_ptr<const Tile>> MultiResolutionTileStore::get(const TileRequest& request) {
  ...
  if (auto tile = memory_->get(request.key)) {
    if (!validate_tile_compatibility(request.key, *tile).ok()) {
      std::lock_guard lock{mutex_};
      ++diagnostics_.corruptions;
  ...
  if (disk_) {
    ...
    if (tile && validate_tile_compatibility(request.key, *tile.value()).ok()) {
...
core::Status MultiResolutionTileStore::put(const TileRequest& request, std::shared_ptr<const Tile> tile) {
  ...
  if (!validation)
    return validation;
  if (!tile || !validate_tile_compatibility(request.key, *tile).ok()) {
...
MultiResolutionTileStore::get_or_produce(const TileRequest& request, ITileProducer& producer,
  ...
  if (cancellation.cancelled())
    return data_error(core::ErrorReason::cancelled, "Tile request was cancelled");
  if (!validate_tile_compatibility(request.key, tile.value()).ok()) {
