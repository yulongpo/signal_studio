...
core::Result<std::uint64_t> SignalDescriptor::frame_bytes() const {
  if (signal_kind != SignalKind::real && signal_kind != SignalKind::complex) {
    return data_error(core::ErrorReason::invalid_argument, "Unknown signal kind");
  }
...
core::Status SignalDescriptor::validate() const {
  ...
  if ((signal_kind != SignalKind::real && signal_kind != SignalKind::complex) ||
      (component_layout != ComponentLayout::real && component_layout != ComponentLayout::interleaved &&
       component_layout != ComponentLayout::planar) ||
      (component_order != ComponentOrder::not_applicable && component_order != ComponentOrder::iq &&
       component_order != ComponentOrder::qi) ||
      (endianness != Endianness::not_applicable && endianness != Endianness::little && endianness != Endianness::big)) {
    return data_error(core::ErrorReason::invalid_argument, "Descriptor contains an unknown enum value");
  }
  for (const auto& [field, field_provenance] : provenance) {
    static_cast<void>(field);
    switch (field_provenance.origin) {
    case FieldOrigin::user:
    case FieldOrigin::sidecar:
    case FieldOrigin::wav_header:
    case FieldOrigin::adapter:
    case FieldOrigin::filename_hint:
      break;
    default:
      return data_error(core::ErrorReason::invalid_argument, "Descriptor contains an unknown field origin");
    }
  }
...
core::Result<double> display_frequency(double baseband_frequency_hz, const SignalDescriptor& descriptor,
  ...
  if (reference != FrequencyReference::absolute) {
    return data_error(core::ErrorReason::invalid_argument, "Unknown frequency reference");
  }
