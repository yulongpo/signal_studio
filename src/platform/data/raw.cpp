...
bool valid_source_format(SourceFormat format) noexcept {
  return format == SourceFormat::raw || format == SourceFormat::wav;
}

bool valid_component_order(ComponentOrder order) noexcept {
  return order == ComponentOrder::not_applicable || order == ComponentOrder::iq || order == ComponentOrder::qi;
}
...
core::Result<detail::WavHeaderFields> detail::calculate_wav_header_fields(const SignalDescriptor& descriptor,
                                                                          std::uint64_t sample_bytes) {
  const auto validation = descriptor.validate();
  if (!validation) {
    return validation;
  }
  constexpr auto maximum_wav_u32 = static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max());
  if (descriptor.component_layout == ComponentLayout::planar || sample_bytes > maximum_wav_u32 - 36U) {
    return data_error(core::ErrorReason::invalid_argument, "WAV export requires interleaved data below RIFF limits");
  }
  const auto scalar = descriptor.scalar_bytes();
  if (!scalar) {
    return scalar.error();
  }
  const auto channels = descriptor.signal_kind == SignalKind::complex ? 2U : 1U;
  const auto sample_rate = descriptor.sample_rate_hz;
  if (std::trunc(sample_rate) != sample_rate || sample_rate > static_cast<double>(maximum_wav_u32)) {
    return data_error(core::ErrorReason::invalid_argument,
                      "WAV export requires an integral sample rate representable as uint32");
  }
  const auto block_align = channels * scalar.value();
  const auto byte_rate = static_cast<std::uint64_t>(sample_rate) * block_align;
  if (block_align > std::numeric_limits<std::uint16_t>::max() || byte_rate > maximum_wav_u32) {
    return data_error(core::ErrorReason::invalid_argument,
                      "WAV export block alignment or byte rate exceeds RIFF limits");
  }
  const auto audio_format =
      descriptor.scalar_type == ScalarType::float32 || descriptor.scalar_type == ScalarType::float64 ? 3U : 1U;
  return WavHeaderFields{static_cast<std::uint32_t>(36U + sample_bytes),
                         static_cast<std::uint16_t>(audio_format),
                         static_cast<std::uint16_t>(channels),
                         static_cast<std::uint32_t>(sample_rate),
                         static_cast<std::uint32_t>(byte_rate),
                         static_cast<std::uint16_t>(block_align),
                         static_cast<std::uint16_t>(scalar.value() * 8U),
                         static_cast<std::uint32_t>(sample_bytes)};
}
...
core::Result<WavDescriptor> read_wav_descriptor(const std::filesystem::path& path, bool confirm_stereo_iq,
  ...
  if (!valid_component_order(confirmed_order)) {
    return data_error(core::ErrorReason::invalid_argument, "Unknown WAV component order");
  }
  ...
  while (cursor + 8U <= file_size.value()) {
    ...
    if (fourcc(chunk_header.value(), "fmt ")) {
      ...
      byte_rate = little_u32(span.subspan(8U, 4U));
  ...
  if (sample_rate == 0U) {
    return data_error(core::ErrorReason::invalid_argument, "WAV sample rate is invalid");
  }
  ...
  if (bits_per_sample == 0U || bits_per_sample % 8U != 0U) {
    return data_error(core::ErrorReason::invalid_argument, "WAV bit depth is not byte aligned");
  }
  const auto bytes_per_sample = static_cast<std::uint64_t>(bits_per_sample / 8U);
  if (bytes_per_sample > std::numeric_limits<std::uint64_t>::max() / channels) {
    return data_error(core::ErrorReason::invalid_argument, "WAV block alignment overflows uint64");
  }
  const auto expected_block_align = static_cast<std::uint64_t>(channels) * bytes_per_sample;
  if (expected_block_align > std::numeric_limits<std::uint16_t>::max() ||
      block_align != static_cast<std::uint16_t>(expected_block_align)) {
    return data_error(core::ErrorReason::invalid_argument, "WAV block alignment is inconsistent with its format");
  }
  if (expected_block_align > std::numeric_limits<std::uint64_t>::max() / sample_rate) {
    return data_error(core::ErrorReason::invalid_argument, "WAV byte rate overflows uint64");
  }
  const auto expected_byte_rate = static_cast<std::uint64_t>(sample_rate) * expected_block_align;
  if (expected_byte_rate > std::numeric_limits<std::uint32_t>::max() ||
      byte_rate != static_cast<std::uint32_t>(expected_byte_rate)) {
    return data_error(core::ErrorReason::invalid_argument, "WAV byte rate is inconsistent with its format");
  }
  if (data_bytes % expected_block_align != 0U) {
    return data_error(core::ErrorReason::invalid_argument, "WAV data is not aligned to complete frames");
  }

  const auto frames = data_bytes / expected_block_align;
...
core::Result<SignalSelection> select_samples(const RawReadResult& browsed, const SampleRange& selection,
  ...
  const auto descriptor_validation = source.descriptor().validate();
  if (!descriptor_validation) {
    return descriptor_validation;
  }
  if (!valid_source_format(source.source_format())) {
    return data_error(core::ErrorReason::invalid_argument, "Unknown data source format");
  }
...
core::Result<ExportReceipt> export_selection(const SignalSelection& selection, const std::filesystem::path& destination,
  ...
  if (!validation)
    return validation;
  if (!valid_source_format(selection.source_format) || selection.data_source_version_id.empty() ||
      selection.samples.size() != selection.source_range.size() ||
  ...
  std::optional<detail::WavHeaderFields> wav_header;
  if (selection.source_format == SourceFormat::wav) {
    auto calculated = detail::calculate_wav_header_fields(selection.source_descriptor, sample_bytes);
    if (!calculated) {
      return calculated.error();
    }
    wav_header = calculated.value();
  }
  ...
  if (selection.source_format == SourceFormat::wav) {
    ...
    append_little_u32(encoded, wav_header->riff_size);
    append_text(encoded, "WAVEfmt ");
    append_little_u32(encoded, 16U);
    append_little_u16(encoded, wav_header->audio_format);
    append_little_u16(encoded, wav_header->channels);
    append_little_u32(encoded, wav_header->sample_rate);
    append_little_u32(encoded, wav_header->byte_rate);
    append_little_u16(encoded, wav_header->block_align);
    append_little_u16(encoded, wav_header->bits_per_sample);
    append_text(encoded, "data");
    append_little_u32(encoded, wav_header->data_size);
  }
