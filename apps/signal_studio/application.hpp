...
class Application final {
 public:
  ...
  [[nodiscard]] core::Result<data::SignalSlice>
  read_samples(const data::FileDataSource& source, std::uint64_t begin, std::uint64_t count,
               std::uint64_t maximum_read_bytes, std::function<bool()> cancel = {});

  /// Welch PSD of a slice. Requires an FFT backend (CUDA in this environment).
  [[nodiscard]] core::Result<dsp::PsdResult>
  analyze_psd(const data::SignalSlice& slice, double sample_rate_hz, std::uint64_t nfft,
