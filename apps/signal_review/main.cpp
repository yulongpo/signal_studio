...
Result<ReviewOutcome> review_wav(const std::filesystem::path& path) {
  ...
  if (!ec && wd->descriptor.frame_bytes().ok() && *wd->descriptor.frame_bytes() > 0) {
    const std::uint64_t usable = wd->data_bytes > 0 ? wd->data_bytes : (file_bytes > wd->descriptor.byte_offset ? file_bytes - wd->descriptor.byte_offset : 0);
  ...
  if (!range.ok()) return Status(range.error());
  ReadRequest req;
  req.range = *range;
...
Result<ReviewOutcome> review_sc16(const std::filesystem::path& path) {
  ...
  if (!hint.has_sr) {
    return Status::failure(signal::core::ErrorCode{signal::core::ErrorDomain::data,
  ...
  if (hint.has_cf) d.center_frequency_hz = hint.cf_hz;
  ...
  if (!facts.ok()) return Status(facts.error());
  ...
  if (!count_range.ok()) return Status(count_range.error());
  ReadRequest req;
...
int main(int argc, char** argv) {
  ...
  if (std::string_view{argv[1]} == "--version") {
    ...
    std::printf("signal_review %u.%u.%u (platform %.*s)\n", bi.version.major, bi.version.minor,
                bi.version.patch, static_cast<int>(bi.product.size()), bi.product.data());
    return 0;
  }
  ...
  Result<ReviewOutcome> result =
      (ext == ".wav") ? review_wav(path) : (ext == ".sc16" || ext == ".raw") ? review_sc16(path) :
                                                                                review_wav(path);
  if (!result.ok()) {
    ReviewOutcome o;
    o.path = path.generic_string();
