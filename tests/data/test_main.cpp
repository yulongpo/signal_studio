...
bool wait_for_condition(Predicate&& predicate, std::chrono::milliseconds timeout = std::chrono::seconds{5}) {
...
std::filesystem::path fixture_root(std::string_view case_id) {
  const auto root = std::filesystem::temp_directory_path() / ("signal-studio-ms01-data-tests-" + std::string{case_id});
...
void rewrite_tile_metadata_with_valid_digest(const std::filesystem::path& path, std::size_t offset, std::byte value) {
  std::ifstream input{path, std::ios::binary};
  require(static_cast<bool>(input), "无法打开磁盘瓦片夹具");
  std::vector<char> characters{std::istreambuf_iterator<char>{input}, std::istreambuf_iterator<char>{}};
  require(!input.bad() && characters.size() > 32U && offset < characters.size() - 32U, "无法读取磁盘瓦片夹具");
  std::vector<std::byte> bytes(characters.size());
  std::transform(characters.begin(), characters.end(), bytes.begin(),
                 [](char byte) { return static_cast<std::byte>(static_cast<unsigned char>(byte)); });
  bytes[offset] = value;
  const auto payload = std::span<const std::byte>{bytes}.first(bytes.size() - 32U);
  const auto digest = signal::core::hash_bytes(payload);
  require(digest, "无法重算磁盘瓦片摘要");
  std::copy(digest.value().bytes.begin(), digest.value().bytes.end(),
            reinterpret_cast<std::uint8_t*>(bytes.data() + bytes.size() - 32U));
...
std::filesystem::path make_wav(const std::filesystem::path& root, std::string_view name = "stereo.wav",
                               std::uint32_t sample_rate = 48'000U, std::uint32_t byte_rate = 48'000U * 4U,
...
public:
  signal::core::Result<Tile> produce(const TileRequest& request, const CancellationToken& cancellation) override {
    ...
    tile.height = request.kind == TileKind::stft ? request.key.pixel_height : 1U;
...
void test_data_closed_loop(const std::filesystem::path& root) {
  ...
  auto unknown_format_selection = selected_real.value();
  unknown_format_selection.source_format = static_cast<SourceFormat>(255U);
  require(!export_selection(unknown_format_selection, root / "unknown-format.raw", 4U),
          "导出入口接受了未知 SourceFormat");
...
void test_descriptor_validation(const std::filesystem::path& root) {
  ...
  auto unknown_kind = complete;
  unknown_kind.signal_kind = static_cast<SignalKind>(255U);
  auto unknown_scalar = complete;
  unknown_scalar.scalar_type = static_cast<ScalarType>(255U);
  auto unknown_layout = complete;
  unknown_layout.component_layout = static_cast<ComponentLayout>(255U);
  auto unknown_order = complete;
  unknown_order.component_order = static_cast<ComponentOrder>(255U);
  auto unknown_endian = complete;
  unknown_endian.endianness = static_cast<Endianness>(255U);
  auto unknown_origin = complete;
  unknown_origin.provenance["scalarType"] = {static_cast<FieldOrigin>(255U), true};
  require(!unknown_kind.validate() && !unknown_scalar.validate() && !unknown_layout.validate() &&
              !unknown_order.validate() && !unknown_endian.validate() && !unknown_origin.validate() &&
              !FileDataSource::open_raw(truncated, unknown_kind, "unknown-kind"),
          "RAW/描述符入口接受了未知枚举值");
}
...
void test_frequency_semantics() {
  ...
  require(display_frequency(125.0, descriptor, FrequencyReference::absolute).value() == 1125.0,
          "绝对频率映射必须保持同一 bin");
  require(!display_frequency(125.0, descriptor, static_cast<FrequencyReference>(255U)),
          "未知 FrequencyReference 被当作绝对频率");
}
...
void test_wav(const std::filesystem::path& root) {
  ...
  require(!read_wav_descriptor(path, true, static_cast<ComponentOrder>(255U)), "WAV 入口接受了未知 ComponentOrder");
  ...
  auto source = FileDataSource::open_wav(path, "wav-export-source", true, ComponentOrder::iq);
  require(source, "WAV 导出测试源打开失败");
  auto browsed = source.value()->read({range(0U, 2U), 8U, {}});
  auto selected = browsed ? select_samples(browsed.value(), range(0U, 2U), *source.value())
                          : signal::core::Result<SignalSelection>{browsed.error()};
  require(selected, "WAV 导出测试选区失败");

  auto invalid_export = selected.value();
  invalid_export.source_descriptor.sample_rate_hz = 48'000.5;
  require(!export_selection(invalid_export, root / "wav-fractional-rate-export.wav", 52U),
          "WAV 导出接受了非整数采样率");
  invalid_export.source_descriptor.sample_rate_hz =
      static_cast<double>(std::numeric_limits<std::uint32_t>::max()) + 1.0;
  require(!export_selection(invalid_export, root / "wav-out-of-range-rate-export.wav", 52U),
          "WAV 导出接受了超出 uint32 的采样率");
  invalid_export.source_descriptor.sample_rate_hz = static_cast<double>(std::numeric_limits<std::uint32_t>::max());
  require(!export_selection(invalid_export, root / "wav-overflow-byte-rate-export.wav", 52U),
          "WAV 导出接受了溢出 uint32 的 byteRate");

  auto boundary_export = selected.value();
  constexpr auto boundary_rate = std::numeric_limits<std::uint32_t>::max() / 4U;
  boundary_export.source_descriptor.sample_rate_hz = static_cast<double>(boundary_rate);
  const auto boundary_path = root / "wav-boundary-byte-rate-export.wav";
  require(export_selection(boundary_export, boundary_path, 52U), "WAV 导出错误拒绝合法 byteRate 上界");
  const auto boundary_wav = read_wav_descriptor(boundary_path, true, ComponentOrder::iq);
  require(boundary_wav && boundary_wav.value().descriptor.sample_rate_hz == static_cast<double>(boundary_rate),
          "WAV 合法 byteRate 上界未按原值写入");
  constexpr auto maximum_riff_data = static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max()) - 36U;
  require(signal::data::detail::calculate_wav_header_fields(selected.value().source_descriptor, maximum_riff_data),
          "WAV 导出错误拒绝合法 RIFF 36+data 上界");
  require(
      !signal::data::detail::calculate_wav_header_fields(selected.value().source_descriptor, maximum_riff_data + 1U),
      "WAV 导出接受了溢出 uint32 的 RIFF 36+data");

  const auto inconsistent_block = make_wav(root, "wav-invalid-block-align.wav", 48'000U, 96'000U, 2U);
  require(!read_wav_descriptor(inconsistent_block, true) &&
              !FileDataSource::open_wav(inconsistent_block, "wav-invalid-block", true),
          "blockAlign 与声道/位深不一致的 WAV 被解析或发布");
  const auto inconsistent_rate = make_wav(root, "wav-invalid-byte-rate.wav", 48'000U, 96'000U, 4U);
  require(!read_wav_descriptor(inconsistent_rate, true) &&
              !FileDataSource::open_wav(inconsistent_rate, "wav-invalid-rate", true),
          "byteRate 与采样率/blockAlign 不一致的 WAV 被解析或发布");
  const auto overflowing_rate = make_wav(root, "wav-overflow-byte-rate.wav", std::numeric_limits<std::uint32_t>::max(),
                                         std::numeric_limits<std::uint32_t>::max(), 4U);
  require(!read_wav_descriptor(overflowing_rate, true) &&
              !FileDataSource::open_wav(overflowing_rate, "wav-overflow-rate", true),
          "byteRate 乘法溢出的 WAV 被解析或发布");
}
...
void test_loader(const std::filesystem::path& root) {
  ...
  const auto scheduling = signal::task::evaluate_scheduling(signal::task::WorkClass::io, 51ms);
  require(scheduling && scheduling.value().must_schedule, "超过 50 ms 的加载 IO 必须由 TaskRuntime 调度");
  ...
  auto empty_cancel_result = IncrementalLoader::create("source-empty-cancel-" + suffix, path, descriptor, plan.value());
  require(empty_cancel_result && empty_cancel_result.value()->cancel_import(), "零完整帧导入取消失败");
  const auto empty_cancelled = empty_cancel_result.value()->snapshot();
  require(empty_cancelled.source_state == DataSourceState::unavailable && !empty_cancelled.published_range &&
              empty_cancelled.next_sample == plan.value().requested_range.begin() && empty_cancelled.bytes_read == 0U &&
              !empty_cancelled.has_remaining_chunks,
          "零完整帧取消不得发布部分范围");

  auto boundary_cancel = IncrementalLoader::create("source-boundary-cancel-" + suffix, path, descriptor, plan.value());
  require(boundary_cancel && boundary_cancel.value()->process_next() && boundary_cancel.value()->cancel_import(),
          "完整帧后取消失败");
  const auto boundary_cancelled = boundary_cancel.value()->snapshot();
  require(boundary_cancelled.source_state == DataSourceState::partial_read_available &&
              boundary_cancelled.published_range && boundary_cancelled.published_range->range() == range(0U, 1U) &&
              boundary_cancelled.published_range->samples().size() == 1U &&
              boundary_cancelled.published_range->samples().complex_values()[0] == values.front() &&
              boundary_cancelled.next_sample == 1U && boundary_cancelled.bytes_read == 4U &&
              !boundary_cancelled.has_remaining_chunks && !boundary_cancel.value()->process_next(),
          "取消未在最后完整帧发布只读部分范围，或发布后仍继续读取");
  ...
  auto progress_dialog = std::make_shared<RecordingTaskObserver>();
  auto task_center = std::make_shared<RecordingTaskObserver>();
  auto task_log = std::make_shared<RecordingTaskObserver>();
  const std::array subscriptions{runtime.add_observer(progress_dialog), runtime.add_observer(task_center),
                                 runtime.add_observer(task_log)};
  ...
  require(runtime.register_handler(
              spec.task_type,
              [loader, plan = plan.value(), &first_chunk, &permit_after_first](signal::task::TaskContext& context) {
                while (loader->snapshot().has_remaining_chunks) {
                  if (!context.checkpoint()) {
                    static_cast<void>(loader->cancel_import());
                    return signal::task::TaskExecutionResult::completed();
                  }
                  const auto status = loader->process_next();
                  if (!status) {
                    const auto log = loader->view_log();
                    return signal::task::TaskExecutionResult::failed(
                        {status.code().stable_text(), std::string{status.message()},
                         "IncrementalLoader::process_next failed", "retry/edit_parameters/cancel_import/view_log", true,
                         "retry", log ? log.value() : "data-load://unavailable/log"});
                  }
                  const auto snapshot = loader->snapshot();
                  const auto progress = plan.target_bytes == 0U ? 1.0
                                                                : static_cast<double>(snapshot.bytes_read) /
                                                                      static_cast<double>(plan.target_bytes);
                  static_cast<void>(context.report_progress(progress, "frame-aligned source loading"));
          ...
          "Data 加载任务处理器注册失败");
  signal::task::ITaskService& task_service = runtime;
  auto handle = task_service.submit(spec);
  require(handle, "ITaskService 加载任务提交失败");
  require(wait_for_condition([&] { return first_chunk.load(); }) && handle.value().pause(),
          "TaskRuntime 未在完整帧边界暂停");
  require(wait_for_condition([&] {
            const auto status = handle.value().status();
            return status && status.value().state == signal::task::TaskState::paused;
          }),
          "TaskRuntime 暂停状态等待超时");
  ...
  require(completed && completed.value().state == signal::task::TaskState::completed &&
              ...
              !loader->snapshot().has_remaining_chunks && !loader->snapshot().error,
          "TaskRuntime 驱动加载未完成");
  const auto progress_events = progress_dialog->snapshot();
  const auto center_events = task_center->snapshot();
  const auto log_events = task_log->snapshot();
  require(!progress_events.empty() && progress_events == center_events && center_events == log_events &&
              std::all_of(progress_events.begin(), progress_events.end(),
                          [&](const auto& event) {
                            return event.status.task_id == spec.task_id && std::isfinite(event.status.progress);
                          }),
          "进度弹窗、任务中心和日志未读取完全相同的 TaskEvent/TaskStatus 状态源");
  ...
  cancel_spec.task_type = "data.load.cancel." + suffix;
  cancel_spec.idempotency_key = "data-load-cancel-" + suffix;
  cancel_spec.provenance.data_source_version_id = {"source-cancel-" + suffix};
  require(runtime.register_handler(cancel_spec.task_type,
                                   [cancel_loader, &cancel_first](signal::task::TaskContext& context) {
                                     while (cancel_loader->snapshot().has_remaining_chunks) {
                                       if (!context.checkpoint()) {
                                         static_cast<void>(cancel_loader->cancel_import());
                                         return signal::task::TaskExecutionResult::completed();
                                       }
                                       const auto status = cancel_loader->process_next();
                                       if (!status) {
                                         const auto log = cancel_loader->view_log();
                                         return signal::task::TaskExecutionResult::failed(
                                             {status.code().stable_text(), std::string{status.message()},
                                              "IncrementalLoader::process_next failed",
                                              "retry/edit_parameters/cancel_import/view_log", true, "retry",
                                              log ? log.value() : "data-load://unavailable/log"});
                                       }
          ...
          "取消导入任务处理器注册失败");
  auto cancel_handle = task_service.submit(cancel_spec);
  require(wait_for_condition([&] { return cancel_first.load(); }) && cancel_handle.value().cancel(),
          "TaskRuntime 取消请求失败");
  ...
  require(cancelled && cancelled.value().state == signal::task::TaskState::canceled &&
              ...
              cancelled_source.published_range &&
              cancelled_source.published_range->range() ==
                  range(plan.value().requested_range.begin(), plan.value().requested_range.begin() + 1U) &&
              cancelled_source.published_range->samples().size() == 1U && !cancelled_source.has_remaining_chunks,
          "TaskRuntime 已取消状态与部分读取可用数据源语义未分离");
  ...
  std::shared_ptr<IncrementalLoader> retry_attempt;
  std::atomic_bool retry_created_fresh{};
  signal::task::TaskSpec bad_spec = spec;
  bad_spec.task_id = {"data-load-failure-" + suffix};
  bad_spec.task_type = "data.load.recovery." + suffix;
  bad_spec.idempotency_key = "data-load-failure-" + suffix;
  bad_spec.max_attempts = 3U;
  require(runtime.register_handler(
              bad_spec.task_type,
              [bad, &retry_attempt, &retry_created_fresh](signal::task::TaskContext& context) {
                auto active = bad;
                if (context.attempt() > 1U) {
                  auto fresh = bad->retry();
                  if (!fresh) {
                    return signal::task::TaskExecutionResult::failed({"DATA.RETRY",
                                                                      "重试创建失败",
                                                                      std::string{fresh.error().message()},
                                                                      "edit_parameters",
                                                                      false,
                                                                      {},
                                                                      "data-load://retry/log"});
                  }
                  retry_attempt = std::shared_ptr<IncrementalLoader>{std::move(fresh.value())};
                  retry_created_fresh.store(retry_attempt.get() != bad.get());
                  active = retry_attempt;
                }
                while (active->snapshot().has_remaining_chunks) {
                  const auto status = active->process_next();
                  if (!status) {
                    const auto log = active->view_log();
                    return signal::task::TaskExecutionResult::failed(
                        {status.code().stable_text(), std::string{status.message()},
                         "IncrementalLoader::process_next failed", "retry/edit_parameters/cancel_import/view_log", true,
                         "retry", log ? log.value() : "data-load://unavailable/log"});
          ...
          "恢复导入任务处理器注册失败");
  auto bad_handle = task_service.submit(bad_spec);
  const auto failed = bad_handle.value().wait();
  const auto failure = bad->snapshot();
  const auto first_log = bad->view_log();
  const auto second_log = bad->view_log();
  require(failed && failed.value().state == signal::task::TaskState::failed && failed.value().failure &&
              failure.error && !failure.published_range && first_log && second_log &&
              first_log.value() == second_log.value() && failed.value().failure->log_link == first_log.value(),
          "失败导入未形成稳定日志 URI 或发布了未验证数据");

  write_bytes(malformed, std::array{std::byte{1}, std::byte{0}, std::byte{2}, std::byte{0}});
  auto retried = task_service.retry(bad_handle.value().id());
  require(retried && retried.value().wait().value().state == signal::task::TaskState::completed &&
              retry_created_fresh.load() && retry_attempt &&
              retry_attempt->snapshot().source_state == DataSourceState::complete,
          "修复输入后的 TaskRuntime retry 未使用全新导入尝试完成");

  auto edited_descriptor = sc16_descriptor(2U);
  LoadPlan edited_plan{range(0U, 2U), 8U, 4U, 4U};
  const auto edited_path =
      make_sc16(root, "loader-edited-" + suffix + ".raw", std::array{ComplexSample{3.0, 4.0}, ComplexSample{5.0, 6.0}});
  auto edited_result = bad->edit_parameters("source-edited-" + suffix, edited_path, edited_descriptor, edited_plan);
  require(edited_result, "edit_parameters 未创建修订后的全新导入尝试");
  auto edited = std::shared_ptr<IncrementalLoader>{std::move(edited_result.value())};
  auto edited_spec = spec;
  edited_spec.task_id = {"data-load-edited-" + suffix};
  edited_spec.task_type = "data.load.edited." + suffix;
  edited_spec.idempotency_key = "data-load-edited-" + suffix;
  edited_spec.provenance.data_source_version_id = {"source-edited-" + suffix};
  require(runtime.register_handler(edited_spec.task_type,
                                   [edited](signal::task::TaskContext&) {
                                     while (edited->snapshot().has_remaining_chunks) {
                                       const auto status = edited->process_next();
                                       if (!status) {
                                         return signal::task::TaskExecutionResult::failed(
                                             {status.code().stable_text(),
                                              std::string{status.message()},
                                              "edited import failed",
                                              "view_log",
                                              false,
                                              {},
                                              "data-load://edited/log"});
                                       }
                                     }
                                     return signal::task::TaskExecutionResult::completed();
                                   }) &&
              task_service.submit(edited_spec).value().wait().value().state == signal::task::TaskState::completed &&
              edited->snapshot().source_state == DataSourceState::complete,
          "修改描述符/计划后的导入未成功");

  require(bad->cancel_import() && !bad->snapshot().published_range && !bad->retry(),
          "cancel_import 未撤销失败导入或仍允许后续重试");
...
void test_pyramid_progressive() {
  ...
  require(!status.transition(static_cast<ProgressiveIndexState>(255U), 1.0, "unknown"),
          "未知 ProgressiveIndexState 被接受");
  ...
  DirectDftTileProducer exact_bound{tone_buffer.view(), range(0U, 8U), 8.0, 8U, 48U};
  DirectDftTileProducer below_bound{tone_buffer.view(), range(0U, 8U), 8.0, 8U, 47U};
  require(exact_bound.produce({TileKind::stft, stft_key, false}, CancellationToken{}) &&
              !below_bound.produce({TileKind::stft, stft_key, false}, CancellationToken{}),
          "STFT maximum_operations 未覆盖 rows*input*bins 精确边界");
  auto excessive_rows = stft_key;
  excessive_rows.pixel_height = 9U;
  require(!dft.produce({TileKind::stft, excessive_rows, false}, CancellationToken{}),
          "STFT 行数超过输入样本后仍被接受");
  CancellationToken cancelled;
  cancelled.cancel();
  require(!dft.produce({TileKind::stft, stft_key, false}, cancelled), "已取消 DFT 请求仍发布瓦片");
}

void test_cache(const std::filesystem::path& root) {
  static std::uint64_t invocation{};
  const auto run = ++invocation;
  auto key = cache_key();
  ...
  require(key.canonical() != incompatible_kind.canonical() && !TileRequest{TileKind::stft, key, false}.validate(),
          "频谱与 STFT 瓦片不得共享缓存身份");
  auto unknown_kind = key;
  unknown_kind.tile_kind = static_cast<TileKind>(255U);
  require(!unknown_kind.validate() && !TileRequest{static_cast<TileKind>(255U), unknown_kind, false}.validate(),
          "未知 TileKind 被缓存或瓦片请求入口接受");
  ...
  const auto valid_tile = Tile{TileKind::spectrum_summary,
                               key.time_viewport,
                               key.frequency_begin_hz,
                               key.frequency_end_hz,
                               key.pixel_width,
                               1U,
                               std::vector<float>(key.pixel_width, 1.0F)};
  const auto shape_directory = root / ("tile-shape-" + std::to_string(run));
  DiskTileStore shape_store{shape_directory, 1'000'000U};
  require(shape_store.recover() && shape_store.put(key, valid_tile), "有效磁盘瓦片写入失败");
  auto wrong_kind_tile = valid_tile;
  wrong_kind_tile.kind = TileKind::stft;
  auto wrong_range_tile = valid_tile;
  wrong_range_tile.time_range = range(1U, 8U);
  auto wrong_frequency_tile = valid_tile;
  ++wrong_frequency_tile.frequency_begin_hz;
  auto wrong_width_tile = valid_tile;
  wrong_width_tile.width = key.pixel_width - 1U;
  wrong_width_tile.values.resize(wrong_width_tile.width);
  auto wrong_height_tile = valid_tile;
  wrong_height_tile.height = 2U;
  wrong_height_tile.values.resize(static_cast<std::size_t>(wrong_height_tile.width) * wrong_height_tile.height);
  require(!shape_store.put(key, wrong_kind_tile) && !shape_store.put(key, wrong_range_tile) &&
              !shape_store.put(key, wrong_frequency_tile) && !shape_store.put(key, wrong_width_tile) &&
              !shape_store.put(key, wrong_height_tile),
          "DiskTileStore::put 接受了与 CacheKey 不相容的瓦片");
  const auto shape_file = std::filesystem::directory_iterator(shape_directory)->path();
  rewrite_tile_metadata_with_valid_digest(shape_file, 45U, std::byte{5});
  require(!shape_store.get(key), "摘要有效但 shape 损坏的磁盘瓦片仍然命中");

  const auto directory = root / ("tiles-" + std::to_string(run));
  ...
  require(!store.put(request, std::make_shared<const Tile>(wrong_width_tile)),
          "MultiResolutionTileStore 接受了错误 width 的瓦片");
  ...
  auto small_memory = MemoryTileCache::create(1'500U, 10U);
  auto first_tile =
      std::make_shared<const Tile>(Tile{TileKind::spectrum_summary, key.time_viewport, key.frequency_begin_hz,
                                        key.frequency_end_hz, 4U, 1U, std::vector<float>(4U, 1.0F)});
  auto second_key = key;
  second_key.parameter_digest = "second";
  auto second_tile =
      std::make_shared<const Tile>(Tile{TileKind::spectrum_summary, key.time_viewport, key.frequency_begin_hz,
                                        key.frequency_end_hz, 4U, 1U, std::vector<float>(4U, 2.0F)});
...
int main(int argc, char** argv) {
  const std::string_view case_id =
      argc == 3 && std::string_view{argv[1]} == "--case" ? std::string_view{argv[2]} : std::string_view{"all"};
  const auto root = fixture_root(case_id);
