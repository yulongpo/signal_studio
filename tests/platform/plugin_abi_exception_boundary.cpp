...
signal_plugin_result_v1 SIGNAL_PLUGIN_CALL guarded_throwing_load(signal_plugin_handle_v1* out_handle)
...
int main() {
  ...
  if (load(&handle) != SIGNAL_PLUGIN_RESULT_INTERNAL_FAILURE_V1)
    return 1;
  unload(handle);
  return 0;
}
