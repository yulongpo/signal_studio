...
int main(int argc, char** argv) {
  ...
  if (query(&host, &plugin) != SIGNAL_PLUGIN_RESULT_OK_V1) return 5;
  if (signal_plugin_validate_api_v1(&host, &plugin) != SIGNAL_PLUGIN_RESULT_OK_V1) return 6;
  signal_plugin_handle_v1 handle = SIGNAL_PLUGIN_NULL_HANDLE;
  if (plugin.load(&handle) != SIGNAL_PLUGIN_RESULT_OK_V1 || plugin.activate(handle) != SIGNAL_PLUGIN_RESULT_OK_V1) return 7;
  plugin.unload(handle);
  host.abi_version += 1u;
  plugin.struct_size = sizeof(plugin);
  if (query(&host, &plugin) != SIGNAL_PLUGIN_RESULT_INCOMPATIBLE_ABI_V1) return 8;
