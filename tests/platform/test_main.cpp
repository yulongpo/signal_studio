...
template <typename Callable> void require_invalid_argument(Callable&& callable, std::string_view message) {
  try {
...
void test_error_contract() {
  ...
  const std::vector<std::pair<ErrorDomain, std::string_view>> stable_domains{
      {ErrorDomain::core, "SS-CORE-E004"},
      {ErrorDomain::data, "SS-DATA-E004"},
      {ErrorDomain::dsp, "SS-DSP-E004"},
      {ErrorDomain::compute, "SS-COMPUTE-E004"},
      {ErrorDomain::task_runtime, "SS-TASK-E004"},
      {ErrorDomain::visualization, "SS-VIS-E004"},
      {ErrorDomain::workbench, "SS-WB-E004"},
      {ErrorDomain::plugin_sdk, "SS-PLG-E004"},
      {ErrorDomain::model_runtime, "SS-MODEL-E004"},
      {ErrorDomain::dataset, "SS-DSET-E004"}};
  ...
  ErrorDetails details{ErrorCategory::resource,
                       ErrorSeverity::error,
                       "Samples unavailable",
                       "mapping failed",
                       {"Select a readable source", "Retry the task"},
                       true,
                       "task-7",
                       "object-3",
                       "data-v2",
                       {{{ErrorDomain::data, ErrorReason::unavailable}, ErrorCategory::resource, "source was removed"}},
                       {"metric://read/7"}};
...
void test_error_invariants() {
  ...
  require_invalid_argument([&] { (void)Status::failure({ErrorDomain::core, ErrorReason::none}, valid_details()); },
...
void test_error_serialization() {
  ...
  ErrorDetails details{
      ...
      "object-1",
      "data-v1",
      {{{ErrorDomain::data, ErrorReason::unavailable}, ErrorCategory::resource, "source mapping unavailable"}},
      {"metric://range"}};
  ...
  require(json.find("\"category\":\"resource\"") != std::string::npos && json.find("unknown") == std::string::npos,
...
void test_capabilities() {
  ...
  require(
      registry.register_capability({"compute.cpu", CapabilityAvailability::available, "portable-cpu", "C++20"}).ok(),
      "registration failed");
...
void test_dependency_graph() {
  const std::map<ModuleId, std::vector<ModuleId>> expected{
      {ModuleId::core, {}},
      {ModuleId::compute, {ModuleId::core}},
      {ModuleId::data, {ModuleId::core}},
  ...
  const std::function<void(ModuleId)> visit = [&](ModuleId id) {
    ...
    if (visited.contains(id))
      return;
    active.insert(id);
    for (const auto dependency : expected.at(id))
      visit(dependency);
    active.erase(id);
    visited.insert(id);
  };
  for (const auto& [id, unused] : expected) {
    (void)unused;
    visit(id);
  }
}
...
int main(int argc, char** argv) {
  ...
  const std::map<std::string, Test> tests{
      {"core.version", test_version},
      {"core.errors.contract", test_error_contract},
      {"core.errors.invariants", test_error_invariants},
      {"core.errors.serialization", test_error_serialization},
      {"core.errors.propagation", test_error_propagation},
      {"core.capabilities", test_capabilities},
      {"core.module-descriptor-validation", test_module_validation},
      {"graph.compatibility", test_dependency_graph},
      {"module.core", [] { test_module(ModuleId::core); }},
      {"module.compute", [] { test_module(ModuleId::compute); }},
      {"module.data", [] { test_module(ModuleId::data); }},
      {"module.task-runtime", [] { test_module(ModuleId::task_runtime); }},
      {"module.dsp", [] { test_module(ModuleId::dsp); }},
      {"module.model-runtime", [] { test_module(ModuleId::model_runtime); }},
      {"module.dataset", [] { test_module(ModuleId::dataset); }},
      {"module.plugin-sdk", [] { test_module(ModuleId::plugin_sdk); }},
#if SIGNAL_STUDIO_TEST_HAS_UI
