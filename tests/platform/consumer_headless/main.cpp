...
int main() {
  ...
  const auto scheduling =
      signal::task::evaluate_scheduling(signal::task::WorkClass::dsp, std::chrono::milliseconds{51});
  if (!workspace || samples.view().size() != 3U || !scheduling || !scheduling.value().must_schedule) {
    return 3;
  }
