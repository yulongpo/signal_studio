...
static signal_plugin_result_v1 SIGNAL_PLUGIN_CALL load(signal_plugin_handle_v1* out) { *out = 1u; return SIGNAL_PLUGIN_RESULT_OK_V1; }
...
static void SIGNAL_PLUGIN_CALL unload(signal_plugin_handle_v1 handle) { (void)handle; }

int main(void) {
  signal_host_api_v1 host = {(uint32_t)sizeof(signal_host_api_v1), SIGNAL_PLUGIN_ABI_V1, NULL, NULL};
  signal_plugin_api_v1 plugin = {(uint32_t)sizeof(signal_plugin_api_v1), SIGNAL_PLUGIN_ABI_V1, "c-consumer", 1u, 0u, 0u,
                                 0u, load, activate, unload};
  if (signal_plugin_validate_api_v1(&host, &plugin) != SIGNAL_PLUGIN_RESULT_OK_V1) return 1;
  host.abi_version += 1u;
