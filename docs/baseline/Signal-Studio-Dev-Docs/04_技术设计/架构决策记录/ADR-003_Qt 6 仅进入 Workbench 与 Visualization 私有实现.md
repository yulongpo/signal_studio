# ADR-003 Qt 6 仅进入 Workbench 与 Visualization 私有实现

| 元数据项 | 内容 |
|---|---|
| 文档编号 | ADR-003 |
| 文档名称 | ADR-003 Qt 6 仅进入 Workbench 与 Visualization 私有实现 |
...
Core/Data/DSP/Compute/Task 不依赖 Qt Widgets；UI 经服务接口消费。

## 备选方案

全栈 Qt 类型会污染 Headless/Python/SDK。
