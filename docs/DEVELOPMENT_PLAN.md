...
对当前实现、BL1.0 的 198 项需求、现有 267 项测试、依赖/预设状态和远程 Git 引用完成
...
- MS-08 按批准基线实现 Signal Generator 薄壳与 Headless CLI，不再以 Signal Review
  作为复用证明宿主。
- 远程 `main` 与 `v1.0.0` 已存在另一条不满足本任务最终验收条件的发布历史。后续不
  合并其实现；MS-09 使用普通合并保留历史并以本验证分支内容为准，不改写既有标签，
  正式发布版本调整为 `1.0.1` / `v1.0.1`。
