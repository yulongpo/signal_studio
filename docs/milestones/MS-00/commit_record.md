...
| 质量修复提交 | 主题 `fix(ms-00): 提升工具链与公共契约稳健性`；作为 `18d2eeb` 之上的新提交创建，不 amend、不 push |
...
质量修复提交的本地门禁为 Debug/Release 全新配置各 45/45、VS Code 本机 Debug 树 45/45 及明确 F5 目标直接启动、三种依赖模式和注入式负向测试、Windows 根路径回归、CI/VS Code 静态校验、基线/外部数据/公共头文件/安装包消费。该提交创建后仍需新的远程 CI 和独立复审；旧运行 `29919175820` 不替代新提交验证。
