...
PSD 估计、平均/保持及“复位连续最大保持”、频谱平滑、STFT 重叠和平滑；高级模式增加帧长、补零、Kaiser/
...
分析前滤波预览复制最多 512 个样本并通过 TaskRuntime 执行，面板定时器只消费完成
状态；切换工程或修改预览参数会递增代际并拒绝旧结果。显示参数即时映射，不重算 DSP，
基带/绝对频率轴可从不可变分析结果反复往返，不累积频率偏移。
自动化覆盖 1280×720、1080P、4K 和 125%～200% DPI；
